...
def _run_nse_quote(symbol: str) -> dict:
    """
    Fetch full quote info from Yahoo Finance for an NSE symbol.
    Raises RuntimeError with a clear message on failure.
    """
    ns_symbol = _to_ns_symbol(symbol)
    logger.info(f"Fetching NSE quote for {ns_symbol}")

    ticker = yf.Ticker(ns_symbol)
    info   = _fetch_info_with_retry(ticker)

    # Attach history for sparkline (non-fatal if fails)
    history = _safe_history(ticker, period="1mo")
    info["_price_history"]  = history
    info["_avg_volume_10d"] = _avg_volume_10d(history)
